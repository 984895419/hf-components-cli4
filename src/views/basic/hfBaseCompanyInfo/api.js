 ...
 * @type {{pageUrl: string}}
 */
export const urlMethods = defaultUrlMethod(baseUrl, 'pkCorp', {
 ...
 * @type {string}
 */
const modelName = 'hfBaseCompanyInfo'
 ...
 * @type {*[]}
 */
const tableFields = ['pkCorp', 'name', 'pkFatherorg', 'creatorUserName', 'createTime', 'modifierUserName', 'modifyTime', 'enableState', 'parentId', 'level', 'levelStr', 'sort', 'deleted']
...
export function formRules(vm) {
  return {
    pkCorp: [
      { required: true, message: vm.$t(getI18nName('pkCorp')) + vm.$t('common.notAllowedNull'), trigger: 'blur' }
    ],
    name: [
