<template>
  <div v-if="showFields">
    ...
    <div v-for="item in showFields" :key="item.value">
              <el-table-column
          v-if="item.value === 'methodId'"
          prop="methodId"
          :label="$t(conf.namespace + '.methodId')"
          min-width="130"
        />
        <el-table-column
          v-if="item.value === 'controllerId'"
          prop="controllerName"
          :label="$t(conf.namespace + '.controllerId')"
          min-width="130"
        />
        <el-table-column
          v-if="item.value === 'value'"
          prop="value"
          :label="$t(conf.namespace + '.value')"
          min-width="130"
        />
        <el-table-column
        ...
        />
        <el-table-column
          v-if="item.value === 'displayName'"
          prop="displayName"
          :label="$t(conf.namespace + '.displayName')"
          min-width="130"
        />
...
<script>
    ...
    export default {
        name: 'BaseHandlerMappingMethodColumns',
