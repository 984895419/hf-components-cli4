<template>
  <el-card class="app-container">
    ...
    <div>
      <simple-search v-model="searchForm" :inline="true" @search="doSearch">
        <template v-slot="{ span }">
          <!-- 新增的的字段配置 -->
          <form-item-col :value="searchForm" :span="span" prop="value" :namespace="conf.namespace" />
          <form-item-col :value="searchForm" :span="span" prop="description" :namespace="conf.namespace" />
          <!-- 字典字段字段设置方法如下
          <form-item-col-dict
            :value="data"
    ...
    <div style="margin-bottom: 10px" class="col-btn-display">
      <base-handler-mapping-method-add :value="{ controllerId: controllerId, enableState: 1 }"
        :action-url="conf.urlMethods.addUrl" @success="doSearch" />
      <div style="float: right" class="col-btn-display">
        <del-btn v-if="conf.urlMethods.deleteUrl
        && toggleRowSelectionArray.length > 0"
          :url="templateUrl(conf.urlMethods.deleteUrl, toggleRowSelectionArray)" :value="toggleRowSelectionArray"
          :label="$t('common.batchDelete')" @success="doSearch" />
        <template-confirm-btn v-if="conf.urlMethods.enableUrl
        && toggleRowSelectionArray.length > 0"
          :url="templateUrl(conf.urlMethods.enableUrl, toggleRowSelectionArray)" :btn-type="'primary'"
    ...
    <table-column-preference-setting-api-slot v-model="showFields" :init-data="tableFields"
      ...
      <template v-slot="{ doSave, preferenceData, headerDragend }">
        <hf-table v-if="showFields" v-loading="loading" :table-data="jsonData.list"
          ...
          <!-- 显示的字段-->
          <base-handler-mapping-method-columns :show-fields="showFields" :url-methods="conf.urlMethods"
            @success="doSearch" />
          <el-table-column fixed="right" :label="$t('common.operate')" width="150">
            ...
            <template slot-scope="scopeRow">
              <div class="col-btn-display">
                <!-- 更新 -->
                <base-handler-mapping-method-update v-permission="['baseHandlerMappingMethod:update']"
                  :value="scopeRow.row" :query-url="conf.urlMethods.queryUrl" :update-url="conf.urlMethods.updateUrl"
                  @success="doSearch" />
                <!-- 删除-->
                <del-btn v-permission="['baseHandlerMappingMethod:delete']"
                  :url="templateUrl(conf.urlMethods.deleteUrl, scopeRow.row)" :btn-type="'text'" :value="scopeRow.row"
                  @success="doSearch" />
                <!-- 查看 -->
                <base-handler-mapping-method-detail :value="scopeRow.row" />
              </div>
            </template>
          </el-table-column>
        </hf-table>
...
import BaseHandlerMappingMethodAdd from './add'
...
import BaseHandlerMappingMethodUpdate from './update'
...
import BaseHandlerMappingMethodDetail from './detail'
import BaseHandlerMappingMethodColumns from './baseHandlerMappingMethodColumns'
...
export default {
  name: 'BaseHandlerMappingMethodIndexVue',
  components: {
    SectionTableColumn,
    TemplateConfirmBtn,
    BaseHandlerMappingMethodColumns,
    BaseHandlerMappingMethodDetail,
    CurdTableColumnSelect,
    DelBtn,
    BaseHandlerMappingMethodUpdate,
    CurdPagination,
    HfTable, BaseHandlerMappingMethodAdd,
    FormItemColDict,
    FormItemCol,
    SimpleSearch,
  ...
  watch: {
    ...
    console.log(val)
      if (val.parentId === 0) {
        ...
        // this.searchForm.methods = val.methods

          this.searchForm.parentId = val.menuId
          this.searchForm.controllerId = null
        //  this.searchForm.controllerId = null
      } else {
         this.searchForm.controllerId = val.controllerId
      }
       this.doSearch()
    }
  },
